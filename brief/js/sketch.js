let posterLayer, txts, posterTxt = `Task
...
function draw() {
	...
	for(let i = 0; i < txts.length; i++) {
		let y = map(i, 0, txts.length, 0, height) + sin(i*15+frameCount*.02)*40
		let x = -width/2 + (frameCount*random(.1, 3)+random(width))%width*1.5
